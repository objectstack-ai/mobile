...
- [ ] 集成 `expo-localization` + `i18next`
- [ ] 创建 `lib/i18n.ts` 配置文件
- [ ] 内置 UI 框架的静态翻译（按钮文字、错误消息、提示等）
- [ ] 支持 RTL 布局（阿拉伯语/希伯来语）
- [ ] 本地化日期、数字、货币格式
- [ ] 用户语言偏好选择
...
- [ ] 使用 `FlashList` 替换 FlatList 实现列表虚拟化
- [ ] 添加图片缓存与懒加载
...
- [ ] 为所有 renderers 和 utility 函数编写单元测试
...
- [ ] 配置 GitHub Actions PR checks（type-check, lint, test）
- [ ] 设置 EAS Build 自动构建（iOS + Android）
- [ ] 配置 EAS Update OTA 更新
- [ ] 设置 App Store / Google Play 部署 via EAS Submit
- [ ] 添加版本号自动化（changesets 或 standard-version）
