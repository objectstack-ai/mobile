...
| **Filter drawer** | ✅ Done | `components/renderers/FilterDrawer.tsx` — dynamic filter UI via QueryBuilder + field definitions |
| **Swipe actions** | ✅ Done | `components/renderers/SwipeableRow.tsx` — Edit (blue) / Delete (red), 80px action width |
| **Record navigation** | ✅ Done | Previous/next in `DetailViewRenderer.tsx` |
| **Widget layout grid** | ✅ Done | `DashboardViewRenderer.tsx` — responsive 2-col (tablet) / 1-col (phone), 600dp breakpoint |
| **Live dashboard data** | ✅ Done | `hooks/useDashboardData.ts` — per-widget useWidgetQuery with aggregation + TanStack Query live updates |
...
- [x] 重构 `hooks/useViewStorage.ts` 对齐 SDK 正式类型（list/form view configs）
- [x] 补全 SavedView 类型定义以匹配 v2.0.1 API
- [x] 更新相关测试用例

#### 4B.2 权限系统 ✅ **解除阻塞 (v2.0.1: `client.permissions.*`)**

- [x] 创建 `hooks/usePermissions.ts`，调用 `client.permissions.getObjectPermissions()`
- [x] 在列表视图中根据权限隐藏/禁用 create 按钮
- [x] 在表单视图中根据字段权限设置只读
- [x] 在详情视图中根据权限隐藏 edit/delete 操作
- [x] 在 ActionBar 中过滤无权限的 actions

#### 4B.3 工作流 & 审批 ✅ **解除阻塞 (v2.0.1: `client.workflow.*`)**

- [x] 创建 `hooks/useWorkflowState.ts`，调用 `client.workflow.getState()`
- [x] 在记录详情页显示当前工作流状态（state badge）
- [x] 渲染可用的状态转换按钮（调用 `client.workflow.transition()`）
- [x] 实现审批流 UI（approve/reject，调用 `client.workflow.approve()` / `reject()`）
- [x] 展示工作流历史记录

#### 4B.4 实时更新 ✅ **解除阻塞 (v2.0.1: `client.realtime.*`)**

- [x] 创建 `hooks/useSubscription.ts`，建立 WebSocket 连接
- [x] 订阅记录变更事件，实现列表实时刷新
- [x] 实现协作指示器（谁正在查看/编辑同一记录）
- [x] 利用 WebSocket 实现实时通知推送

#### 4B.5 推送通知 ✅ **解除阻塞 (v2.0.1: `client.notifications.*`)**

- [x] 配置 `expo-notifications`，获取设备 push token
- [x] 调用 `client.notifications.registerDevice()` 注册 token 到服务端
- [x] 调用 `client.notifications.list()` 获取通知列表
- [x] 构建通知列表 UI（已读/未读状态）
- [x] 实现从通知 deep link 到对应记录/视图
- [x] 调用 `client.notifications.updatePreferences()` 管理通知偏好
...
| **v0.6-beta** ✅ | Phase 4B：权限 + 工作流 + 实时 + 推送通知 | ✅ SDK v2.0.1 | ✅ 已完成 |
| **v0.7-beta** ✅ | Phase 5B：AI 集成 + 服务端 i18n + SDK hooks 自建 | ✅ SDK v2.0.1 | ✅ 已完成 |
| **v1.0-RC** ✅ | Phase 6：监控 + 分析 + 安全审计 + 性能基准 + 发布准备 | Phase 4A+5A 完成 | ✅ 大部分完成（待 E2E 全量通过） |
| **v1.2** | Phase N1–N2：增强通知中心 + Activity Feed + 记录评论 | SDK notification 增强 + `client.activity.*` + `client.comments.*` | 5–7 周 |
| **v1.3** | Phase N3：消息 & 频道（Slack/Teams 模式） | `client.messaging.*` | 4–6 周 |

> 详细设计见 [MESSAGING-NOTIFICATION-CENTER-DESIGN.md](./MESSAGING-NOTIFICATION-CENTER-DESIGN.md)

### 关键路径

- **Mobile 独立开发**（4A → 5A → 6）：✅ **已基本完成**，仅余 E2E 全量通过
- **Phase 4B + 5B**：✅ **已全部完成**（493 项测试通过，58 套件）
- **总计到 v1.0 GA**：仅余 E2E 全量通过 + 真机性能测试 + App Store 发布准备
- **v1.2 通知 + Activity Feed**：需 SDK 团队提供 `client.activity.*` 和 `client.comments.*` API
- **v1.3 消息 & 频道**：需 SDK 团队提供 `client.messaging.*` API
